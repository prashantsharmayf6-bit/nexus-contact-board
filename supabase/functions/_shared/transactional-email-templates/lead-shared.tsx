...
const LeadSharedEmail = ({ leadName, sharedBy }: Props) => (
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>A lead has been shared with you on {SITE_NAME}</Preview>
    <Body style={main}>
      <Container style={container}>
        <Img src={LOGO_URL} alt={SITE_NAME} width="48" height="48" style={logo} />
        <Heading style={h1}>Lead Shared With You</Heading>
        <Text style={text}>
          {sharedBy ? `${sharedBy} has` : 'Someone has'} shared the lead
          <strong> {leadName || 'a lead'}</strong> with you in {SITE_NAME}.
        </Text>
        <Text style={text}>
          Log in to view the lead and collaborate with your team.
        </Text>
...
export const template = {
  component: LeadSharedEmail,
  subject: (data: Record<string, any>) => `Lead "${data.leadName || 'A Lead'}" shared with you`,
  displayName: 'Lead shared',
  previewData: { leadName: 'Prashant Sharma', sharedBy: 'Admin User' },
